...
const randomRange = (min: number, max: number) => floor(random() * (max - min) + min)
const randomElement = <T>(array: T[]): T => array[floor(random() * array.length)]

type UpDown = { up: XY | null; down: XY | null }
...
export default class Dungeon {
  ...
  maxRoomArea: number
  addStairsUp: boolean
  addStairsDown: boolean
  rooms: Room[]
  roomGrid: Room[][][]

  getStairs(): UpDown {
    const result: UpDown = { up: null, down: null }
  ...
  getCollisionMap(): number[][] {
    ...
    const collisionMap: number[][] = Array(this.size.y)
    for (let y = 0; y < this.size.y; y++) {
  ...
  roomIntersect(room1: Room, room2: Room): boolean {
    const x1 = room1.pos.x
    const y1 = room1.pos.y
    const w1 = room1.size.x
    const h1 = room1.size.y

    const x2 = room2.pos.x
    const y2 = room2.pos.y
    const w2 = room2.size.x
    const h2 = room2.size.y
    ...
    const checkRoomList = function (x: number, y: number, rg: Room[][][]) {
      ...
      for (let i = 0; i < r.length; i++) {
        ...
        if (r[i] !== room && touchingRooms.indexOf(r[i]) < 0) {
          ...
          const lx = x - r[i].pos.x
          const ly = y - r[i].pos.y
  ...
  findNewDoorLocation(room1: Room, room2: Room): XY {
  ...
  findRoomAttachment(
    ...
    const r = randomElement(this.rooms)
  ...
  addDoor(doorPos: XY): void {
    ...
    for (let i = 0; i < rooms.length; i++) {
      ...
      const x = doorPos.x - r.pos.x
      const y = doorPos.y - r.pos.y
