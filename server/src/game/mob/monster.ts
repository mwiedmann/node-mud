...
import { createMOB, MOB, MOBUpdateNotes } from './mob'
import { RollResult } from '../combat'

export type Monster = MOB & {
  moveRange: number
  playerRangeToActivate: number
  startingX: number
  startingY: number
  deathTick: number
  reviveTick: number
  ...
  takeDamage(this: Monster, tick: number, roll: RollResult): void
  moveTowardsDestination(this: Monster, tick: number, level: Level<unknown>, notes: MOBUpdateNotes): MOBUpdateNotes
}

export function createMonster(type: MOBType, team: number, id: number, name?: string): Monster {
  const mob = createMOB(type, team, id, name)
  const monster: Monster = {
    ...
    takeDamage(tick: number, roll: RollResult): void {
      mob.takeDamage.call(this, tick, roll)
    ...
    update(tick: number, level: Level<unknown>): MOBUpdateNotes {
      ...
      return mob.update.call(this, tick, level)
    },
    ...
    moveTowardsDestination(tick: number, level: Level<unknown>, notes: MOBUpdateNotes): MOBUpdateNotes {
      ...
      mob.moveTowardsDestination.call(this, tick, level, notes)
