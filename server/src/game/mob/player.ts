...
import { createMOB, MOB, MOBItems } from './mob'
import { PlayerProfession, PlayerRace, WeaponDetails } from 'dng-shared'
import { MOBSkills } from '.'

export type Player<T> = MOB & {
  race: PlayerRace
  profession: PlayerProfession
  startingProfessionSettings: Partial<MOBSkills> & Partial<MOBItems>
  professionProgression: LevelProgression[]
  raceProgression: LevelProgression[]
  connection: T
  lastTickReceivedState: number
  movedLevels: boolean
  kills: number
  damageDone: number
  gold: number
...
export function createPlayer<T>(
  ...
  const mob = createMOB('player', team, id, name)

  const player: Player<T> = {
    ...mob,
    race,
    profession,
    startingProfessionSettings,
    ...
    getState(tick: number, selfId: number): string | undefined {
      ...
      return mob.getState.call(this, tick, selfId, {
        race: this.race,
